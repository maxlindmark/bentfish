...
test <- df_full %>% filter(Year == "2016", Cruise == "BITS")
...
bits_2019_q4_cod <- read.csv("data/stomach_data/2019/COD_2019_Q4_BITS.csv", sep = ";")
bits_2019_q4_fle <- read.csv("data/stomach_data/2019/FLE_2019_Q4_BITS.csv", sep = ";")
bits_2020_q1_cod <- read.csv("data/stomach_data/2020/COD_2020_Q1_BITS.csv", sep = ";")
bits_2020_q1_fle <- read.csv("data/stomach_data/2020/FLE_2020_Q1_BITS.csv", sep = ";")
...
str(bits_2019_q4_cod)
str(bits_2019_q4_cod)
str(bits_2020_q1_cod)
str(bits_2020_q1_cod)

# Bind them together, then see what columns are missing from the base data
data.frame(bits_2017_q4_fle = colnames(bits_2017_q4_fle)[1:32],
           bits_2018_q1_cod = colnames(bits_2018_q1_cod)[1:32],
           bits_2018_q1_fle = colnames(bits_2018_q1_fle)[1:32],
           bits_2018_q4_cod = colnames(bits_2018_q4_cod)[1:32],
           bits_2019_q4_cod = colnames(bits_2019_q4_cod)[1:32],
           bits_2019_q4_fle = colnames(bits_2019_q4_fle)[1:32],
           bits_2020_q1_cod = colnames(bits_2020_q1_cod)[1:32],
           bits_2020_q1_fle = colnames(bits_2020_q1_fle)[1:32]
           )

# Looks like we can rbind them
temp_new <- rbind(bits_2017_q4_fle[1:32],
                  bits_2018_q1_cod[1:32],
                  bits_2018_q1_fle[1:32],
                  bits_2018_q4_cod[1:32],
                  bits_2019_q4_cod[1:32], 
                  bits_2019_q4_fle[1:32], 
                  bits_2020_q1_cod[1:32],
                  bits_2020_q1_fle[1:32]
                  )
...
# [1] "square"               "SD"                   "month"                "comment"             
# [5] "parasites.in.stomach" "cruise"               "Size.group.code"      "Gall.bladder"        
# [9] "stomach.state"
...
# > missing_cols
#  [1] "Age"                    "comments"               "Date"                   "Depth.catch"           
#  [5] "Depthstep"              "Gonad.weight.roundfish" "Ices.rect"              "Index"                 
#  [9] "Lat"                    "Liver.weight.roundfish" "Long"                   "Number"                
# [13] "Perc.stomac.content"    "Processed"              "Q_Year"                 "Sample.type"           
# [17] "source"                 "Stomach.content"        "transect"             
...
temp_new %>% filter(Quarter == 1 & Year == 2020 & HN == 58 & Predator.code == "FLE")
...
ggplot(bits_bio_data, aes(Sample, Fishno)) + geom_point()
...
# Only 839 rows without bio info. Perhaps not that much!
...
# Again, 839 rows without any match in the trawl data
## THIS IS BECAUSE THE PREDATOR CODE IS MISSING IN SOME TABS IN EXCEL!!! NEED TO CLEAN EACH CSV 
...
bias_2019_q4_cod <- read.csv("data/stomach_data/2019/COD_2019_BIAS.csv", sep = ";")

colnames(bias_2018_q4_cod)
colnames(bias_2019_q4_cod)

bias_2018_2019 <- rbind(bias_2018_q4_cod[, 1:32], bias_2019_q4_cod[, 1:32])

temp_new <- bias_2018_2019 %>% rename("SubDiv" = "SD",
                                      "Square" = "square",
                                      "Month" = "month",
                                      "Comment" = "comment",
                                      "Parasites.in.stomach" = "parasites.in.stomach",
...
temp_new_sub$Day <- as.integer(temp_new_sub$Day)
...
```{r add new sub-area, message=FALSE, warning=FALSE}
# Add ICES subdivisions
shape <- shapefile("data/ICES_StatRec_mapto_ICES_Areas/StatRec_map_Areas_Full_20170124.shp")

# For that we need lat and long. We don't have that for the old data. I will therefore 
# use the mapplots::ices.rect(rectangle) to get the coordinates of the centre of the lat long
# First add a column saying if coordinates are true of they are base in rectangle centre
#full_dat <-
full_dat <- full_dat %>% 
  mutate(Lat = replace_na(Lat, -99),
         Long = replace_na(Long, -99)) %>% 
  mutate(coord_from_ices_rect_centre =
         ifelse(Lat == -99 & Long == -99 & !Ices.rect %in% c("", "NANANA"), "Y", "N"))

# This gives a DF of lat and long, change to a vector then add in!!
latlon_df <- mapplots::ices.rect(full_dat$Ices.rect)
  
full_dat <- full_dat %>% mutate(Lat = ifelse(Lat == -99, latlon_df$lat, Lat),
                                Long = ifelse(Long == -99, latlon_df$lon, Long))

# The below function does not work with NAs. Use the function on the non NA coordinates,
# then bind_rows()-them
...
full_dat_coord <- full_dat %>% filter(!Lat == -99 & !Long == -99)

pts <- SpatialPoints(cbind(full_dat_coord$Long, full_dat_coord$Lat), 
                     proj4string = CRS(proj4string(shape)))

full_dat_coord$SubDiv <- over(pts, shape)$Area_27

# Rename subdivisions to the more common names and do some more filtering (by sub div and area)
sort(unique(full_dat_coord$SubDiv))

full_dat_coord <- full_dat_coord %>% 
  mutate(SubDiv = factor(SubDiv),
...
ggplot(full_dat_coord, aes(x = Long, y = Lat, color = SubDiv)) +
  geom_point(size = 0.5) +
  coord_cartesian(expand = 0) +
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
  coord_sf(xlim = c(14, 21), ylim = c(54.5, 59.5)) + 
  theme_classic(base_size = 20) +
  NULL

# Now do bind-rows to get back in the data that do not have sub-division info
full_dat2 <- bind_rows(full_dat_coord, filter(full_dat, Lat == -99 & Long == -99))

nrow(full_dat2)
nrow(full_dat)

# Replace 99 coordinates with NA
full_dat2 <- full_dat2 %>%
  mutate(Long = ifelse(Long == -99, NA, Long),
         Lat = ifelse(Lat == -99, NA, Lat))
...
```{r, save}
sort(unique(full_dat2$Year))
write.csv(full_dat2, "data/for_analysis/stomach/full_stomach_data_21.10.25.csv")

# Only recent years
sub_full_dat <- filter(full_dat2, Year > 2014)
write.csv(sub_full_dat, "data/for_analysis/stomach/2015-2018_stomach_data_21.10.25.csv")
```
