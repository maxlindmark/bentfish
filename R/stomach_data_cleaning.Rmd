...
full_dat$sub_area <- as.numeric(full_dat$SubDiv)
full_dat$sub_area <- as.factor(full_dat$sub_area)

unique(full_dat$sub_area)

full_dat <- full_dat %>% mutate(sub_area = fct_recode(sub_area,
                                                      "1" = "24",
                                                      "2" = "25",
                                                      "3" = "26",
                                                      "4" = "27",
...
# Now modify them...
full_dat <- full_dat %>%
  mutate(sub_area = ifelse(Lat > 56.18 & Long < 18, "4", sub_area)) %>% 
  mutate(sub_area = ifelse(Long > 17.6 & Lat >= 56.49, "5", sub_area)) %>% 
  mutate(sub_area = ifelse(Long > 17 & Lat < 56.49, "3", sub_area))
