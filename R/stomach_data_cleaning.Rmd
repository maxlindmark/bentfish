...
full_dat <- full_dat %>% drop_na(Lat) %>% drop_na(Long)

pts <- SpatialPoints(cbind(full_dat$Long, full_dat$Lat), 
                     proj4string = CRS(proj4string(shape)))

full_dat$SubDiv <- over(pts, shape)$Area_27

# Rename subdivisions to the more common names and do some more filtering (by sub div and area)
sort(unique(full_dat$SubDiv))
...
ggplot(full_dat, aes(x = Long, y = Lat, color = SubDiv)) +
  geom_point(size = 0.5) +
  coord_cartesian(expand = 0) +
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
...
```{r, save}
write.csv(full_dat, "data/for_analysis/stomach/full_stomach_data_21.10.25.csv")
