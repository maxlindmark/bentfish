...
# Now modify them...
full_dat <- full_dat %>%
  mutate(sub_area = ifelse(Lat > 56.18 & Long < 18, 27, sub_area)) %>% 
  mutate(sub_area = ifelse(Long > 17.6 & Lat >= 56.49, 28, sub_area)) %>% 
  mutate(sub_area = ifelse(Long > 17 & Lat < 56.49, 26, sub_area))

ymin = 55; ymax = 58; xmin = 12.5; xmax = 20

ggplot(full_dat, aes(x = Long, y = Lat, color = factor(sub_area))) +
  geom_point(size = 0.5) +
  coord_cartesian(expand = 0) +
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
