...
dat <- dat %>% drop_na(sub_area) %>% mutate(sub_area = as.factor(sub_area))
...
dat2 <- dat %>%
  rename("depth" = "depth_rast",
         ...
         "lon" = "Longitud",
         "lat" = "Latitud") %>% 
  dplyr::select(year, month, day, lon, lat, abundance, biomass, species_group, depth, prov_nr, provID, sub_area, SubDiv, taxa)
