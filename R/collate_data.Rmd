...
Not needed! We use this info in the prediction grid.

```{r add ICES areas, message=FALSE, results="hide"}
# # Now plot the data points and colour according to ICES rectangle
# dat$ices_rect <- dat(lon = dat$lon, lat = dat$lat)
# 
# colourCount = length(unique(dat$ices_rect))
# getPalette = colorRampPalette(brewer.pal(9, "Dark2"))
# 
# ggplot(dat, aes(x = lon, y = lat, color = ices_rect)) +
#   geom_point() +
...
# 
# #dat <- dat %>% drop_na(SubDiv) 
# 
# ggplot(dat, aes(x = lon, y = lat, color = SubDiv)) +
#   geom_point() +
...
#   coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) + 
#   theme_classic(base_size = 20) +
#   geom_point(data = d, aes(Longitud, Latitud), inherit.aes = FALSE, color = "black", size = 0.2) + # add data! 
#   NULL
# 
# # Now do a manual area, similar to sub divisions
# dat$sub_area <- dat$SubDiv
# 
...
# 
# # Now modify them...
# dat <- dat %>% mutate(sub_area = ifelse(lat > 56.15 & sub_area == 2, "3", sub_area))
# dat <- dat %>% mutate(sub_area = ifelse(lon > 17.55 & sub_area == 3, "4", sub_area))
# dat <- dat %>% mutate(sub_area = ifelse(lat > 57.4 & lon < 17.7 & sub_area == 4, "3", sub_area))
# dat <- dat %>% mutate(sub_area = ifelse(lat > 55.5 & sub_area == 1, "2", sub_area))
# dat <- dat %>% mutate(sub_area = ifelse(lat < 55.5 & lon > 14.3, "2", sub_area))
# 
# ggplot(dat, aes(x = lon, y = lat, color = sub_area)) +
#   geom_point(size = 0.5) +
#   coord_cartesian(expand = 0) +
#   scale_color_brewer(palette = "Dark2") +
#   geom_sf(data = world, inherit.aes = F, size = 0.2) +
...
#   NULL
# 
# ggsave("figures/sub_areas_predgrid.png", width = 9, height = 9, dpi = 600)
```
...
dat2 <- dat %>%
  rename("depth" = "depth_rast",
         ...
         "month" = "Month",
         "day" = "Day") %>% 
  dplyr::select(year, month, day, lon, lat, abundance, biomass, species_group, depth, oxy, temp, prov_nr, provID)
