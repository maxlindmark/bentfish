...
p1a <- pred_grid2 %>% drop_na(sub_area) %>% 
  ggplot(., aes(x = lon, y = lat, fill = sub_area)) +
  geom_raster() +
...
p1b <- d %>% drop_na(sub_area) %>%
  ggplot(., aes(x = lon, y = lat, color = sub_area)) +
  geom_point() +
  coord_cartesian(expand = 0) +
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) + 
...
p1c <- dd %>% drop_na(sub_area) %>%
  ggplot(., aes(x = Long, y = Lat, color = sub_area)) +
  geom_point() +
  coord_cartesian(expand = 0) +
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) + 
