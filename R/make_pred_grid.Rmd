...
# Filter the months in the spring, which is what we use for this analysis, so that the grid isn't unnecesarily large, and the years after 1983 for coverage
d <- d %>% filter(artal > 1983 & Month > 3 & Month < 7)

# Generate the coordinates
pred_grid <- expand.grid(
  lat = seq(55, 60.5, by = 0.05),
  lon = seq(13, 20, by = 0.05))

tf <- exclude.too.far(pred_grid$lon, pred_grid$lat, d$Longitud, d$Latitud, 0.03)
...
pred_grid %>% 
  ...
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) +
  geom_point(data = d, aes(Longitud, Latitud), alpha = 0.3,
             inherit.aes = FALSE, color = "red", size = 0.1) +
  NULL

# Just remove the grids just north of Gotland...
pred_grid <- pred_grid %>% mutate(too_far = ifelse(lat > 58 & lon > 18, TRUE, too_far))
...
# Will fix the land issue once we add in the depth variable
```
...
# Filter away land and too far grids
pred_grid <- pred_grid %>% filter(too_far == FALSE) %>% filter(depth > 0)
...
pred_grid2$sub_area <- pred_grid2$SubDiv

pred_grid2 <- pred_grid2 %>% mutate(sub_area = fct_recode(sub_area,
                                                          "1" = "24",
                                                          "2" = "25",
                                                          "3" = "27",
                                                          "4" = "28"))
...
# Now modify them...
pred_grid2 <- pred_grid2 %>% mutate(sub_area = ifelse(lat > 56.15 & sub_area == 2, "3", sub_area))
pred_grid2 <- pred_grid2 %>% mutate(sub_area = ifelse(lon > 17.55 & sub_area == 3, "4", sub_area))
pred_grid2 <- pred_grid2 %>% mutate(sub_area = ifelse(lat > 57.4 & lon < 17.7 & sub_area == 4, "3", sub_area))
pred_grid2 <- pred_grid2 %>% mutate(sub_area = ifelse(lat > 55.5 & sub_area == 1, "2", sub_area))
pred_grid2 <- pred_grid2 %>% mutate(sub_area = ifelse(lat < 55.5 & lon > 14.3, "2", sub_area))

ggplot(pred_grid2, aes(x = lon, y = lat, color = sub_area)) +
  geom_point(size = 0.5) +
  coord_cartesian(expand = 0) +
  scale_color_brewer(palette = "Dark2") +
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
